...
- **update_element_summaries:** Run after `update_core` when you need per-player history and future fixtures. Use `--mode top --n 250` for faster runs; use `--mode all` for full backfill.
...
| **players** | Elements: id, web_name, team_id, element_type_id, now_cost, total_points, selected_by_percent, form, xG/xA, etc. |
